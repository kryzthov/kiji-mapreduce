...
public final class KijiPivot extends JobTool<KijiPivotJobBuilder> {
  ...
  protected void validateFlags() throws Exception {
    ...
    if (mPivoter.isEmpty()) {
      throw new RequiredFlagException("table-mapper");
    }
  ...
  protected void configure(KijiPivotJobBuilder jobBuilder)
    ...
    if (!(input instanceof KijiTableMapReduceJobInput)) {
      throw new RuntimeException(String.format(
          "Invalid table-mapper job input: '%s'; must input from a Kiji table.", mInputFlag));
    }
    ...
    if (!(output instanceof KijiTableMapReduceJobOutput)) {
      throw new RuntimeException(String.format(
          "Invalid table-mapper job output: '%s'; must output to a Kiji table.", mOutputFlag));
    }
